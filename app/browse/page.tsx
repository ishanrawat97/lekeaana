  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--paper)' }}>
      ...
      <div style={{ padding: '32px 40px 20px', borderBottom: '1px solid var(--rule)' }}>
        ...
        <h1 className="serif-d" style={{ fontSize: 44, margin: 0, lineHeight: 1.05 }}>
          Pick what you can carry.
        </h1>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', minHeight: 'calc(100vh - 180px)' }}>
        {/* Sidebar */}
        ...
        <div style={{ padding: '20px 40px 40px' }}>
          ...
          {loading ? (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {[...Array(6)].map((_, i) => (
                <div key={i} style={{ border: '1px solid var(--rule)', background: 'var(--paper-2)', height: 160, animation: 'pulse 1.5s ease-in-out infinite' }} />
              ))}
            </div>
          ) : requests.length === 0 ? (
              ...
              {requests.map(r => {
                ...
                return (
                  <div key={r.id} style={{ border: '1px solid var(--rule)', background: 'var(--paper)', padding: 18, position: 'relative' }}>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--ink-3)', marginBottom: 0 }}>
                      Qty {r.quantity} · ${r.price_usd} retail · {r.from_country}
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16, paddingTop: 12, borderTop: '1px dashed var(--rule-2)' }}>
                      <div>
